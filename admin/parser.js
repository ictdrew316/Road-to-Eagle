...
(function (global) {
  ...
  function build(files) {
    ...
    for (const [mid, rec] of scouts.entries()) {
      ...
      for (const gname in ALT_GROUPS) {
        let earnedOpt = null, earnedDate = null;
        for (const o of ALT_GROUPS[gname]) {
          if (o in rec.approved) { earnedOpt = o; earnedDate = rec.approved[o].date; break; }
        }
        if (earnedOpt) er++;
        checklist.push({ type: "group", group: gname, options: ALT_GROUPS[gname], earned: earnedOpt, date: earnedDate });
      }
